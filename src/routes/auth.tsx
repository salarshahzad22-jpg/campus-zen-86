...
export const Route = createFileRoute("/auth")({
  ...
  head: () => ({
    meta: [
      { title: "Sign in — Campus Zen" },
      { name: "description", content: "Sign in or create your Campus Zen student account." },
      { property: "og:title", content: "Sign in — Campus Zen" },
      { property: "og:description", content: "Sign in or create your Campus Zen student account." },
...
function AuthPage() {
  ...
  const [view, setView] = useState<View>(search.mode ?? "signin");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (view === "signup") {
        ...
        if (error) throw error;
        if (data.session) {
          toast.success("Account created — welcome to Campus Zen!");
      ...
      else toast.error(err instanceof Error ? err.message : "Authentication failed");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      <div className="hidden lg:flex flex-col justify-between p-10 bg-gradient-to-br from-primary to-accent text-primary-foreground">
        ...
        <div>
          <h2 className="text-3xl font-bold">Everything a student needs, in one place.</h2>
          <p className="mt-4 text-primary-foreground/90 max-w-md">
            Assignments, attendance, exams, resources and an AI study assistant — all in a clean,
            mobile-friendly dashboard.
          </p>
        </div>
        <p className="text-sm opacity-80">© Campus Zen</p>
      </div>
        ...
        <Card className="w-full max-w-md p-8">
          ...
          {pendingVerification ? (
            <div className="text-center">
              ...
              <Button variant="outline" className="mt-6 w-full" onClick={resendVerification}>
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className={view === "forgot" ? "mt-6 space-y-4" : "space-y-4"}>
                {view === "signup" && (
                  <div>
                ...
                {view === "signin" && (
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" className="text-sm text-primary hover:underline" onClick={() => setView("forgot")}>
                      Forgot password?
                    </button>
                  </div>
                )}
              ...
              <p className="mt-6 text-center text-sm text-muted-foreground">
                {view === "signup" ? "Already have an account? " : view === "forgot" ? "Remembered it? " : "New here? "}
                <button
                  type="button"
                  className="font-medium text-primary hover:underline"
                  onClick={() => setView(view === "signup" ? "signin" : view === "forgot" ? "signin" : "signup")}
                >
                  {view === "signup" || view === "forgot" ? "Sign in" : "Create an account"}
                </button>
              </p>
            </>
          )}
